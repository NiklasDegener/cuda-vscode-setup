from src.gemv_triton import check_and_launch_gemv, gemv_kernel
import torch
import triton
...
def rand_vec(m, dtype=torch.float32):
    return (torch.rand((m), device=DEVICE, dtype=dtype) - 0.5).contiguous()
...
def bench_square(min_sz=128, max_sz=2048, step=128, dtype=torch.float32, warmup=5, iters=20):
    ...
    for s in range(min_sz, max_sz + 1, step):
        ...
        c_tri = check_and_launch_gemv(a, b)
        ...
        tri_ms = time_gpu(lambda: gemv_kernel[grid](a, b, c, M, N), warmup=warmup, iters=iters)
